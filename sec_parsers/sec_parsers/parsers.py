from sec_parsers.string_detector_groups import (
    HeaderStringDetectorGroup,
    SEC_10K_StringDetectorGroup,
    SEC_8K_StringDetectorGroup,
    SEC_S1_StringDetectorGroup,
    SEC_DEF14A_StringDetectorGroup,
)
from sec_parsers.xml_helper import (
    get_text,
    get_all_text,
...
from sec_parsers.element_detector_groups import (
    HeaderElementDetectorGroup,
    SEC_10K_ElementGroup,
    SEC_8K_ElementGroup,
    SEC_DEF14A_ElementGroup,
)
...
class HTMLParser:
    def __init__(self, **kwargs):
        ...
        self.color_dict = {
            "remove;": "#f2f2f2",
            "skip;": "#FAFAD2",
        }  # If you want certain parsing types to have certain colors

        self._init(**kwargs)
        self.all_detectors = (
            self.element_detector_group.element_detectors
            + self.string_detector_group.string_detectors
        )
        self.sort_detectors()
    ...
    def sort_detectors(self):
        ...
        continue_detectors = [
            detector
            for detector in self.all_detectors
            if detector.parsing_rule == "continue"
        ]
    ...
    def detect_style_from_string(self, string, rule_list=[]):
        ...
        if rule_list:  # WIP
            string_detectors = [
                ...
                if detector.parsing_rule in rule_list
            ]
    ...
    def clean_parse(self, html):
        ...
        parsed_elements_to_pop = html.xpath(
            "//*[@parsing_string='']"
        ...
        remove_strings = [
            item.style for item in self.all_detectors if item.cleaning_rule == "remove;"
        ]
        skip_strings = [
            item.style for item in self.all_detectors if item.cleaning_rule == "skip;"
        ]
    ...
    def visualize(self, html):
        ...
        parsing_values = list(
            set([element.attrib["parsing_type"] for element in elements])
        )
        color_dict = dict(
            zip(parsing_values, headers_colors_list[: len(parsing_values)])
        )
    ...
    def construct_xml_tree(self, html, metadata, add_parsing_id=False):
        ...
        levels_dict = {
            item.style: item.level for item in self.all_detectors if item.level != -1
        }
        ...
        flag = True
        remove_elem = None
        for current_event, current_elem in etree.iterwalk(
            html, events=("start", "end")
        ):  # change to iterwalk to skip certain elements
            if current_event == "start":
                ...
                if flag:  # handle introduction
                    if current_parsing_type in base_headers:
                        ...
                        last_section_parsing_id = str(
                            current_elem.attrib.get("parsing_id")
                        )
                    ...
                    if current_parsing_type == "skip;":
                        ...
                        if level is not None:  # WIP
                            ...
                            parsing_type_list = [
                                item.attrib["parsing_type"] for item in stack
                            ]
...
class SEC_DEF14A_Parser(HTMLParser):
    def _init(self, **kwargs):
        ...
        self.element_detector_group = SEC_DEF14A_ElementGroup()
