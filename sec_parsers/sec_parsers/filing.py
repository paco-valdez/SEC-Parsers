...
from sec_parsers.parsers import (
    SEC_10K_Parser,
    SEC_10Q_Parser,
    SEC_8K_Parser,
    SEC_S1_Parser,
...
class Filing:
    def __init__(self, html, filing_type=None):
        self.metadata = {}
    ...
    def _to_xml(self, add_parsing_id=False):
        self.xml = self.parser.construct_xml_tree(
            html=self.html, metadata=self.metadata, add_parsing_id=add_parsing_id
        )
    ...
    def get_title_tree(self, node=None, level=0, attribute="title"):
        ...
        if (
            "signatures" in tree_atrib.lower()
        ):  # workaround for adding signatures section but not signature parsing
            pass
        else:
            for child in node:
                tree_atrib += (
                    "\n"
                    + "|-" * level
                    + self.get_title_tree(child, level + 1, attribute)
                )
