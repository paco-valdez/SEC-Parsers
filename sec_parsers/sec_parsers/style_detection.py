...
def detect_emphasis_capitalization(string):  # WIP
    ...
    if string.lower() in [
        item.lower() for item in ["None", "None.", "Omitted.", "Not Applicable."]
    ]:
        return False
...
def detect_type(node):
    """Detects if a type is a type."""
    if node.tag == "type":
        return True
    return False


def detect_sequence(node):
    """Detects if a type is a sequence."""
    if node.tag == "sequence":
        return True
    return False


def detect_filename(node):
    """Detects if a type is a filename."""
    if node.tag == "filename":
        return True
    return False


def detect_description(node):
    """Detects if a type is a description."""
    if node.tag == "description":
        return True
    return False


def detect_text(node):
    """Detects if a type is a text."""
    if node.tag == "text":
        return True
    return False


def detect_schedule14A(string):
    """e.g. SCHEDULE 14A"""
    match = re.search(r"^SCHEDULE 14A$", string, re.IGNORECASE)
    if match:
        return True
    return False
